...
def main(page: ft.Page):
    ...
    # Termo de busca atual (lista de 1 elemento para poder alterar dentro de closures)
    search_query = [""]

    # ---------- Helpers de dados (busca + ordenação) ----------
    ...
    def build_task_row(t: dict, atualizar_contador) -> ft.Container:
        ...
        def alternar_concluida(e):
            ...
            # Atualiza a cor do título imediatamente (sem precisar reconstruir a lista toda,
            # já que marcar/desmarcar não muda a prioridade nem a ordenação)
    ...
    def view_lista() -> ft.View:
        # Texto do contador "X de Y tarefas concluídas" (fica no topo, acima da busca)
        ...
        # Preenche contador e lista pela primeira vez (sem precisar de page.update aqui,
        # pois route_change() chama page.update() ao final da construção das views)
        atualizar_contador()
        for t in tarefas_visiveis():
            lista_view.controls.append(build_task_row(t, atualizar_contador))
    ...
    def view_nova(task: dict | None = None) -> ft.View:
        ...
        # o mesmo formulário usado para criar uma tarefa nova.
